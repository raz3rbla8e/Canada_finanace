...
class TestPreMigrationDatabase:
    ...
    def test_sets_version_to_1(self, raw_db):
        self._create_pre_migration_db(raw_db)
        run_migrations(raw_db)
        assert _get_schema_version(raw_db) == 8
...
class TestFutureMigration:
    def test_new_migration_runs_on_existing_db(self, raw_db):
        ...
        # Simulate a v9 migration that adds a column
        def _migrate_v9(db):
        ...
        try:
            dbmod.MIGRATIONS.append((9, "add label column", _migrate_v9))
            dbmod.LATEST_VERSION = 9
            run_migrations(raw_db)
            assert _get_schema_version(raw_db) == 9
    ...
    def test_new_migration_skips_already_applied(self, raw_db):
        """A v9 migration should not run if the DB is already at v9."""
        run_migrations(raw_db)
        call_count = 0
